...
export class ArenaLevelGenerator extends AbstractLevelGenerator {
     ...
     */
    public fillLevelWithVoronoiPoints(level: LevelModel, config: IFillLevelWithVoronoiPointConfig): void {
        const {
            targetCellType,
            cellAllowedToChange,
        } = config;
         ...
         */
        const voronoiPoints: Array<Position & {type: string}> = randomPoints.map((item: Position, index: number) => {
        ...
        levelCells.forEach((examinedCell: Cell) => {
            ...
            const validateBorderX: boolean = examinedCellX === 0 || examinedCellX === globalConfig.LEVEL_WIDTH - 1;
            const validateBorderY: boolean = examinedCellY === 0 || examinedCellY === globalConfig.LEVEL_HEIGHT - 1;
