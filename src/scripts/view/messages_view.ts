...
export class MessagesView {
  ...
  private messagesListElement: HTMLUListElement = document.getElementById(
    'messagesList',
  ) as HTMLUListElement;
  ...
  public addMessage(
    ...
    if (this.messagesList.length > 0) {
      ...
      if (sameMessageCount > 1) {
        (messagesListElement.lastChild as HTMLLIElement).innerText = `${
  ...
  public appendMessage(text: string): void {
    ...
    this.messagesListElement.scrollTop = (
      this.messagesListElement.lastChild as HTMLElement
    ).offsetTop;
  }
